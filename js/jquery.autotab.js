...
(function ($) {
  var platform = navigator.platform,
    settings = {
      tabPause   : 800,
      focusChange: null,
      iOS        : (platform === 'iPad' || platform === 'iPhone' || platform
      === 'iPod'),
  ...
  var getSettings = function (e) {
    var settings = {
      ...
      originalValue: '',
      changed      : false,
      editable     : (e.type === 'text' || e.type === 'password' || e.type
      === 'textarea' || e.type === 'tel' || e.type === 'number' || e.type
      === 'email' || e.type === 'search' || e.type === 'url'),
      filterable   : (e.type === 'text' || e.type === 'password' || e.type
      === 'textarea'),
      tabOnSelect  : false
    };
    // If $.autotab.selectFilterByClas is true and the format not specified,
    // automatically select an element's format based on a matching class name.
    // The first matched element becomes the selected format for the filter.
    if ($.autotab.selectFilterByClass === true
      && typeof $(e).data('autotab-format') === 'undefined') {
      var classes = ['all', 'text', 'alpha', 'number', 'numeric',
        'alphanumeric', 'hex', 'hexadecimal', 'custom'];
  ...
  var queryObject = function (e) {
    return (typeof e !== 'undefined' && (typeof e === 'string' || !(e
    instanceof jQuery)));
  };
  var getSelection = function (e) {
    ...
    if (e.type === 'text' || e.type === 'password' || e.type === 'textarea') {
      if (typeof e.selectionStart === 'number' && typeof e.selectionEnd
        === 'number') {
  ...
  $.fn.autotab = function (method, options) {
    ...
    // Remove hidden fields since tabbing backwards is supported on different
    // form elements
    ...
    if (method == 'filter') {
      ...
      for (var i = 0, length = filtered.length; i < length; i++) {
        var defaults = getSettings(filtered[i]),
          newOptions = options;
        // Retain the established target/previous values as this area is for
        // filtering only
    ...
    // Disable auto tab and filtering
      ...
      if (method == 'remove' || method == 'destroy' || method == 'disable') {
        for (var i = 0, length = filtered.length; i < length; i++) {
          var defaults = getSettings(filtered[i]);
          defaults.disabled = true;
          setSettings(filtered[i], defaults);
      ...
      // Re-enable auto tab and filtering
        ...
        if (method == 'restore' || method == 'enable') {
          for (var i = 0, length = filtered.length; i < length; i++) {
            var defaults = getSettings(filtered[i]);
        ...
        // Refresh target/previous elements
          ...
          if (method == 'refresh') {
            for (var i = 0, length = filtered.length; i < length; i++) {
              ...
              // Nothing was specified for the target element, so automatically
              // set it
              if (defaults.target === null || defaults.target.selector === '') {
                ...
                if (typeof defaults.target === 'string'
                  ...
                  defaults.target = $(typeof defaults.target === 'string' ?
                                      defaults.target :
                                      defaults.target.selector);
              ...
              // Nothing was specified for the previous element, so
              ...
              if (defaults.previous === null || defaults.previous.selector
                ...
                if (typeof defaults.previous === 'string'
                  || defaults.previous.selector) {
                  defaults.previous = $(typeof defaults.previous === 'string' ?
                                        defaults.previous :
                                        defaults.previous.selector);
  ...
  var autotabBind = function (element, options) {
    ...
    // Sets targets to element based on the name or ID passed if they are not
    // currently objects
    if (queryObject(defaults.target)) {
      defaults.target = $(defaults.target);
    }
    ...
    if (typeof element.maxLength === 'undefined' && element.type
      == 'textarea') {
    ...
    // Add a change event to select lists only so that we can auto tab when a
    // value is selected
    ...
    $(element).on('autotab-next', function (event, defaults) {
      ...
      setTimeout(function () {
        ...
        if (!defaults.disabled && target.length) {
          // Using focus on iOS devices is a pain, so use the browser's
          // next/previous buttons to proceed
          if (!settings.iOS) {
            ...
            if (target.prop('disabled') || target.prop('readonly')) {
              ...
              // Allows the user to navigate between each charater with arrow
              // keys
      ...
      setTimeout(function () {
        ...
        if (!defaults.disabled && previous.length) {
          ...
          if (previous.prop('disabled') || previous.prop('readonly')) {
            ...
            if (value.length && previous.data('autotab-editable')
              && !defaults.arrowKey) {
              if (settings.ie11) {
              ...
              if (settings.ie11) {
                previous.val(value).focus().select();
              }
              else {
                previous.focus().val(value).select();
              }
            }
      ...
      function (e) {
        ...
        if (keyCode == 8) {
          ...
          if (keyCode == 9 && settings.focusChange !== null) {
            // Tab backwards
            if (e.shiftKey) {
              settings.focusChange = null;
              return;
            }
            if ((new Date().getTime() - settings.focusChange.getTime())
              < settings.tabPause) {
              settings.focusChange = null;
              return false;
            }
            ...
            if (keyCode == 13) {
              ...
              if (this.type !== 'range' && this.type !== 'select-one'
                && this.type
                !== 'select-multiple') {
                if ((this.type !== 'tel' && this.type !== 'number')
                  || ((this.type
                  === 'tel' || this.type === 'number') && this.value.length
                  == 0)) {
                  if (keyCode == 37 && (!defaults.editable || selection.start
                    == 0)) {
                    defaults.arrowKey = true;
                    $(this).trigger('autotab-previous', defaults);
                  }
                  else
                    if (keyCode == 39 && (!defaults.editable
                      || !defaults.filterable || selection.end
                      == this.value.length || this.value.length == 0)) {
                      defaults.arrowKey = true;
                      $(this).trigger('autotab-next', defaults);
                    }
                }
        ...
        if (!defaults || defaults.disabled || (settings.firefox && e.charCode
          === 0) || e.ctrlKey || e.altKey || keyCode == 13 || this.disabled) {
          return true;
        }
        var keyChar = String.fromCharCode(keyCode);
        if (this.type != 'text' && this.type != 'password' && this.type
          != 'textarea') {
          // this.value.length is the length before the keypress event was
          // trigged
        ...
        if (defaults.trigger !== null && defaults.trigger.indexOf(keyChar)
          >= 0) {
          if (settings.focusChange !== null && (new Date().getTime()
            - settings.focusChange.getTime()) < settings.tabPause) {
            settings.focusChange = null;
          }
          else {
            defaults.arrowKey = false;
        ...
        settings.focusChange = null;
        var hasValue = document.selection && document.selection.createRange ?
                       true : (keyCode > 0);
        keyChar = filterValue(this, keyChar, defaults);
        if (hasValue && (keyChar === null || keyChar === '')) {
          return false;
        }
        // Many, many thanks to Tim Down for this solution:
        // http://stackoverflow.com/a/3923320/94656
        if (hasValue && (this.value.length <= this.maxLength)) {
          ...
          if (selection.start === 0 && selection.end == this.value.length) {
            this.value = keyChar;
            setSettings(this,
              {changed: (this.value != defaults.originalValue)});
          }
          else {
            if (this.value.length == this.maxLength && selection.start
              === selection.end) {
              defaults.arrowKey = false;
              $(this).trigger('autotab-next', defaults);
              return false;
            }
            this.value = this.value.slice(0, selection.start) + keyChar
              + this.value.slice(selection.end);
            setSettings(this,
              {changed: (this.value != defaults.originalValue)});
          }
          // Prevents the cursor position from being set to the end of the text
          // box This is called even if the text is fully selected and replaced
          // due to an unexpected behavior in IE6 and up (#32)
      ...
      on('drop paste', function (e) {
        ...
        (function (e, originDefaults) {
          setTimeout(function () {
            ...
            e.value = filterValue(e, e.value, originDefaults).substr(0,
              originDefaults.maxlength);
            var handlePaste = function (e, previousValue) {
              ...
              for (var i = 0, count = previousValue.length; i < count; i++) {
                lastIndex
                  = hiddenInput.value.indexOf(previousValue.charAt(i).toLowerCase(),
                    lastIndex) + 1;
              }
              var trimmedValue = hiddenInput.originalValue.substr(lastIndex),
                filteredValue = filterValue(e, trimmedValue, defaults).substr(0,
                  defaults.maxlength);
              if (!filteredValue) {
                return;
              }
              ...
              if (filteredValue.length == defaults.maxlength) {
                ...
                // Firefox causes all but the first and last elements to retain
                // a select all state, so in order to effectively support arrow
                // keys, the starting point of the selection is to the last
                // possible cursor
                if (settings.firefox) {
                  setTimeout(function () {
